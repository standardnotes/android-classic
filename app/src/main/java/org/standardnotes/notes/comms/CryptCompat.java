...
//    }
//
    final static IvParameterSpec blankIvSpec = new IvParameterSpec(new byte[] {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00});
//
...
//    }
    ...
    public static byte[] AES128CBCDecrypt(byte[] base64Data, byte[] hexKey, byte[] ivSpec) throws Exception {
        Cipher ecipher = Cipher.getInstance(AES_CBC_PKCS5_PADDING);
        SecretKey sks = new SecretKeySpec(hexKey, AES);
        ecipher.init(Cipher.DECRYPT_MODE, sks, ivSpec == null ? blankIvSpec : new IvParameterSpec(ivSpec));
        byte[] resultData = ecipher.doFinal(base64Data);
        return resultData;
    }

    public static byte[] AES128CBCEncrypt(byte[] data, byte[] hexKey, byte[] ivSpec) throws Exception {
        Cipher ecipher = Cipher.getInstance(AES_CBC_PKCS5_PADDING);
        SecretKey sks = new SecretKeySpec(hexKey, AES);
        ecipher.init(Cipher.ENCRYPT_MODE, sks, ivSpec == null ? blankIvSpec : new IvParameterSpec(ivSpec));
        byte[] resultData = ecipher.doFinal(data);
        return resultData;
    }
...
//    }
//
    public static byte[] HMAC256(byte[] contentData, byte[] akHexData) throws NoSuchAlgorithmException, InvalidKeyException {
...
//        byte[] akHexData = Hex.decode(ak);
        ...
        return sha256_HMAC.doFinal(contentData);
    }

    public static byte[] SHA1(byte[] textBytes) throws NoSuchAlgorithmException, UnsupportedEncodingException {
